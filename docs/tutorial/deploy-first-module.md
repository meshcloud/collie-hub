...
with some additional files like `documentation.tf` and a `template` folder. We will explain what these are for in a later step of the turotial. For now it's enough to understand that a kit module is just a standard terraform module.
...
applied module kit/azure/bootstrap to foundations/test/platforms/az/bootstrap
...
**❯ Azure Organization Hierarchy azure/bootstrap**
...
parsing kit modules ...
parsing kit modules DONE 24ms
 ? Select a kit module from your repository › Azure Organization Hierarchy azure/bootstrap
 ? Select a foundation › likvid-prod
 ? Select a platform › az
applied module kit/azure/bootstrap to foundations/likvid-prod/platforms/az/bootstrap

Tip: edit the terragrunt configuration invoking the kit module at foundations/test/platforms/az/bootstrap/terragrunt.hcl
```
